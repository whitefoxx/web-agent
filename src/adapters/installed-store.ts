 ...
 *
 * Schema lives in the shared "web-agent" DB, bumped to v2 to add the
...
export interface InstalledAdapter {
  ...
  /** For explore-synthesized adapters: the verify ("test-run") outcome so the UI can
...
export async function setInstalledEnabled(id: string, enabled: boolean): Promise<void> {
  if (!hasIndexedDb()) return;
  // Single-transaction RMW (F-35 class): read + mutate + write in ONE readwrite
  // tx so a concurrent putInstalled (a reinstall, or setInstalledVerify) can't
  // land between the two-transaction read and write and clobber fresh fields.
  await tx('readwrite', async (store) => {
    const row = (await reqAsPromise(store.get(id))) as InstalledAdapter | undefined;
    if (!row) return;
    row.enabled = enabled;
...
/** Record an explore adapter's verify ("test-run") outcome. */
