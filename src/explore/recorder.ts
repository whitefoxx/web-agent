...
export class Recorder {
  ...
  /** Resume recording into an EXISTING trace (Explore v2 resume): reuse its
   * metadata (status → 'recording') and continue `seq` from where it left off so
   * appended events don't collide with the [traceId, seq] keys already stored. */
  static async resume(
    meta: TraceMeta,
    startSeq: number,
    opts: RecorderOptions = {},
  ...
  /** The seq the NEXT recorded event will get = count of events so far. Used
   * as a cursor to slice "events since the last synthesis" (explore v2). */
  get nextSeq(): number {
    return this.seq;
  }
