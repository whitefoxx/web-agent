...
/** Curated `Object` shim exposed to expressions INSTEAD of the real `Object`.
 * The reflection / prototype methods (getPrototypeOf, getOwnPropertyDescriptor,
 * defineProperty, setPrototypeOf, create, getOwnPropertyNames, …) are OMITTED:
 * an expression can chain them to take `'constructor'` as a STRING ARGUMENT —
 * bypassing BLOCKED_KEYS, which only guards member-access *keys* — and reach the
 * real `Function` constructor (CSP blocks the eval escalation, but
 * `Object.defineProperty(Object.getPrototypeOf([]), …)` is plain prototype
 * pollution across the SW realm, no eval needed). Only safe enumeration/merge
 * helpers are exposed. See docs/health-audit-2026-07.md Tier3-#15. */
const SafeObject = {
  keys: Object.keys,
 ...
 * `Function`, `eval`, `globalThis`, and only a curated `Object` (see SafeObject). */
...
export interface FetchStepDef {
  url: Expr;
  method?: string;
  /** Query params appended to the URL (opencli feature). Each value is an Expr. */
...
async function doFetch(
  ...
  let url = String(evaluateExpr(f.url, ctx));
  // F-6: honor `fetch.params` (opencli query params). v2ex topic/member/user/node/
  // replies + hackernews/search put query args here; the shim previously dropped
  // them, so e.g. /api/topics/show.json went out with NO ?id= → 404 (the v1 API is
  // alive; it just needs the param), and hackernews/search ignored its query.
  if (f.params) {
    const qs = new URLSearchParams();
    for (const [k, v] of Object.entries(f.params)) {
      const val = evaluateExpr(v, ctx);
      if (val !== undefined && val !== null && val !== '') qs.append(k, String(val));
    }
    const q = qs.toString();
...
async function executeFetch(
  ...
  // A PRIMARY fetch that returned a single OBJECT (profile/user-style endpoints —
  // bluesky getProfile, HN /user/<id>.json) with NO prior rows: treat the object
  // as ONE row so a following `map` can transform it. Without this the object is
  // dropped and `map` runs over the empty prior rows → []. A side-fetch that
  // augments an existing list (rows already populated) keeps its rows.
  // See docs/tests findings F-11.
  if (rows.length === 0 && data !== null && typeof data === 'object') {
    return [data];
  }
  return rows;
}
