...
export function toolActivity(trace: ToolTrace): Activity {
  ...
  switch (tool) {
    case 'update_plan':
      return { icon: 'plan', label: '更新计划' };
    case 'submit_plan':
      return { icon: 'plan', label: '提交计划' };
    case 'spawn_subagent':
      return { icon: 'subagent', label: `子任务:${clip(str(a.task), 24)}` };
    case 'remember':
      return { icon: 'memory', label: `记住:${clip(str(a.fact), 24)}` };
    case 'view_image':
      return { icon: 'image', label: '查看图片' };
    case 'generate_image':
      return { icon: 'image', label: '生成图片' };
  }

  // Generic web primitives.
  switch (tool) {
    case 'generic__open_url':
      return { icon: 'navigate', label: `打开 ${shortUrl(str(a.url))}` };
    case 'generic__get_page_text':
      return { icon: 'read', label: '读取页面文本' };
    case 'generic__get_text_from_tab':
      return { icon: 'read', label: '读取标签页文本' };
    case 'generic__get_interactives':
      return { icon: 'read', label: '读取可交互元素' };
    case 'generic__screenshot':
      return { icon: 'camera', label: '截图' };
    case 'generic__click':
      return { icon: 'click', label: '点击' };
    case 'generic__click_by_text':
      return { icon: 'click', label: `点击 "${clip(str(a.text), 18)}"` };
    case 'generic__type_into':
      return { icon: 'type', label: a.text ? `输入 "${clip(str(a.text), 18)}"` : '输入文本' };
    case 'generic__scroll_page':
      return { icon: 'scroll', label: '滚动页面' };
    case 'generic__close_tab':
      return { icon: 'navigate', label: '关闭标签页' };
  }
  ...
  if (/search|搜索/i.test(name) || query) {
    return {
      icon: 'search',
      label: query ? `在 ${site || '页面'} 搜索 "${clip(query, 22)}"` : `${site} · ${name}`,
    };
  }
  if (site) return { icon: 'site', label: `${site} · ${name}` };
  return { icon: 'action', label: name || tool || '执行操作' };
}
 ...
 * card ("涉及站点"). Scans the goal + steps text for known site names and
