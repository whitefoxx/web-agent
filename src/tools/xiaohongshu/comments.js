...
import { AuthRequiredError, CliError, EmptyResultError } from '../../runtime/errors.js';
...
cli({
    ...
    columns: ['rank', 'comment_id', 'parent_id', 'author', 'text', 'likes', 'time', 'is_reply', 'reply_to'],
    func: async (page, kwargs) => {
      ...
      (async () => {
        ...
        // A comment's stable XHS id lives in its element id: id="comment-<hex>".
        const commentId = (el) => {
          const m = el && String(el.id || '').match(/comment-([0-9a-z]+)/i)
          return m ? m[1] : ''
        }
        ...
        for (const p of parents) {
          ...
          const id = commentId(item) || commentId(p)

          if (!text) continue
          results.push({ comment_id: id, parent_id: '', author, text, likes, time, is_reply: false, reply_to: '' })
          ...
          if (withReplies) {
            ...
            p.querySelectorAll('.reply-container .comment-item-sub, .sub-comment-list .comment-item').forEach(sub => {
              ...
              if (!sText) return
              results.push({ comment_id: commentId(sub), parent_id: id, author: sAuthor, text: sText, likes: sLikes, time: sTime, is_reply: true, reply_to: author })
            })
          }
        }
