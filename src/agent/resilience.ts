 ...
 * (propagating the first one's `reason`). Used to bound an LLM call by BOTH the
 * caller's signal (user Stop) AND a timeout, so a stalled endpoint can't hang
 * the loop forever. Filters out undefined; returns the lone signal unwrapped;
 * with none, returns a signal that never aborts. */
...
export class ThrashTracker {
  ...
  record(key: string, ok: boolean): string | null {
    ...
    if (n >= this.cfg.maxSameFailure) {
      return `The same tool call has failed ${n} times in a row; tripped the breaker to avoid spinning (${key.slice(0, 100)}).`;
    }
    return null;
  }
}
...
export class NoProgressTracker {
  ...
  record(completed: number, anyToolSuccess: boolean): string | null {
    ...
    if (this.stalls >= this.cfg.maxStalls) {
      return `No plan progress and no successful operations for ${this.stalls} turns in a row; looks stuck, pausing.`;
    }
    return null;
  }
}
