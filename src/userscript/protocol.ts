...
export interface InitMsg {
  ...
  /** Secret env vars to expose as `process.env.*` to this adapter run ONLY.
   * Resolved in the SW from the secret vault (chrome.storage.local) by scanning
   * the source for `process.env.NAME` references — the model never sees these.
   * The isolated USER_SCRIPT world means the host page's JS can't read them off
   * the port message either. Reset per run so one adapter's key can't leak into
   * the next (same reused world). See secret-store.ts + docs/adapter-secrets.md. */
