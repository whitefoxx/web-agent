...
export function configureWebWorld(): Promise<boolean> {
...
/** One runner per tab at a time. The dispatcher leases an EXCLUSIVE tab per call
 * from the per-site pool (parallel-execution v3), so two installed-func calls
 * never share a tab; this per-tab map is the backstop that keeps each tab's
 * runner port bound to a single session. */
...
export function handleRunnerPortConnect(port: chrome.runtime.Port): void {
  ...
  // ("expected web-userscript-runner, got X" — vs nothing at all).
...
async function diagnose(
  ...
  try {
    const raw = await ev.call(
      page,
      `JSON.stringify({
        status: document.documentElement?.getAttribute('data-web-runner') ?? null,
        at: document.documentElement?.getAttribute('data-web-runner-at') ?? null,
        extra: document.documentElement?.getAttribute('data-web-runner-extra') ?? null,
      })`,
    );
...
export interface RunInstalledFuncArgs {
  ...
  /** Secret env vars to expose as `process.env.*` for this run (resolved by the
   * dispatcher from the vault by scanning `source`). Threaded into INIT.env. */
...
export async function runInstalledFuncAdapter(
  ...
  const worldOk = await configureWebWorld();
  if (!worldOk) {
