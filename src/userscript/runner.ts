...
(() => {
  ...
  function mark(status: string, extra?: Record<string, unknown>): void {
    try {
      ...
      if (!el) return;
      el.setAttribute('data-web-runner', status);
      el.setAttribute('data-web-runner-at', String(Date.now()));
      if (extra) el.setAttribute('data-web-runner-extra', JSON.stringify(extra));
    } catch {
      /* hostile DOM; ignore */
    }
  }
  ...
  if (!rt || typeof rt.connect !== 'function') {
    ...
    try {
      console.warn('[web-runner] chrome.runtime.connect not available — adapter cannot run.');
    } catch {
      /* ignore */
    }
    return;
  ...
  try {
    ...
    try {
      console.warn('[web-runner] chrome.runtime.connect threw:', e);
    } catch {
      /* ignore */
    }
    return;
