/**
 * Port of opencli's clis/linkedin/timeline.test.js.
 * Pure-helper assertions via __test__ + command-shape assertions.
 */
import { describe, expect, it } from 'vitest';
import { findAdapter } from '@base/runtime/registry.js';
import '../../../marketplace/linkedin/timeline.js';

const { parseMetric, buildPostId, mergeTimelinePosts } = (
  await import('../../../marketplace/linkedin/timeline.js')
).__test__ as any;

describe('linkedin timeline adapter', () => {
  const command = findAdapter('linkedin', 'timeline');
  ...
  it('has limit arg with default 20', () => {
    const limitArg = command!.args!.find((a: any) => a.name === 'limit');
