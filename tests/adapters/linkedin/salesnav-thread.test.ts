/**
 * Port of opencli's clis/linkedin/salesnav-thread.test.js.
 */
import { describe, expect, it } from 'vitest';
import { ArgumentError, CommandExecutionError } from '../../../src/runtime/errors.js';
import '../../../marketplace/linkedin/salesnav-thread.js';
...
const {
  ...
  threadMatchesInput,
  salesnavThreadUrl,
} = (await import('../../../marketplace/linkedin/salesnav-thread.js')).__test__ as any;

describe('linkedin salesnav-thread command', () => {
  it('accepts Sales Navigator inbox URLs, raw thread ids, lead URLs, urns, and names', () => {
    expect(parseThreadInput('https://www.linkedin.com/sales/inbox/2-abc%3D%3D')).toEqual([
      'thread_id',
      '2-abc==',
    ]);
    expect(parseThreadInput('2-abc==')).toEqual(['thread_id', '2-abc==']);
    expect(parseThreadInput('https://www.linkedin.com/sales/lead/P1,NAME_SEARCH,T1')).toEqual([
      'recipient_urn',
      'urn:li:fs_salesProfile:(P1,NAME_SEARCH,T1)',
    ]);
    expect(parseThreadInput('urn:li:fs_salesProfile:(P1,NAME_SEARCH,T1)')).toEqual([
      'recipient_urn',
      'urn:li:fs_salesProfile:(P1,NAME_SEARCH,T1)',
    ]);
    expect(parseThreadInput('Rachael Stolberg')).toEqual(['name', 'rachael stolberg']);
    expect(() => parseThreadInput('urn:li:fs_salesProfile:(P1,undefined,T1)')).toThrow(
      ArgumentError,
    );
    expect(() => parseThreadInput('https://www.linkedin.com/sales/lead/P1,undefined,T1')).toThrow(
      ArgumentError,
    );
  });
  ...
  it('parses thread messages in chronological order with sender names and timestamps', () => {
    const rows = parseSalesnavThreadMessages({
      ...
      messages: [
        ...
        {
          id: 'm1',
          author: 'urn:li:fs_salesProfile:(SELF,NAME_SEARCH,T2)',
          subject: 'Quick question',
          body: 'Hi Rachael',
          deliveredAt: 1777188013523,
          type: 'INMAIL',
        },
      ],
    });
  ...
  it('fails typed on malformed thread message payloads', () => {
    ...
    expect(() => parseSalesnavThreadMessages({ id: '2-thread', messages: [null] })).toThrow(
  ...
  it('matches recipient identifiers against inbox rows', () => {
    const row = {
      ...
      participants: [
        { name: 'Rachael Stolberg', entity_urn: 'urn:li:fs_salesProfile:(P1,NAME_SEARCH,T1)' },
      ],
    };
    expect(threadMatchesInput(row, ['thread_id', '2-thread'])).toBe(true);
    expect(
      threadMatchesInput(row, ['recipient_urn', 'urn:li:fs_salesProfile:(P1,NAME_SEARCH,T1)']),
    ).toBe(true);
