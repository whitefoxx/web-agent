/**
 * Port of opencli's clis/linkedin/thread-snapshot.test.js.
...
import '../../../marketplace/linkedin/thread-snapshot.js';

const { canonicalizeLinkedInThreadUrl, parseMaxScrolls } = (
  await import('../../../marketplace/linkedin/thread-snapshot.js')
).__test__ as any;

function makeFakePage(snapshot: unknown) {
  return {
...
describe('linkedin thread-snapshot command', () => {
  it('accepts only exact LinkedIn messaging thread URLs', () => {
    expect(
      canonicalizeLinkedInThreadUrl(
        'https://www.linkedin.com/messaging/thread/2-abc==/?mini=true#x',
      ),
  ...
  it('registers as a read command for loading full thread context', () => {
    const command = findAdapter('linkedin', 'thread-snapshot');
    expect(command).toBeDefined();
    expect(command!.access).toBe('read');
    expect(command!.columns).toEqual(
      expect.arrayContaining(['thread_url', 'recipient', 'message_count', 'latest_text']),
    );
  });
