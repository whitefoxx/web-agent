...
const {
  ...
  normalizeProject,
} = (await import('../../../marketplace/linkedin/profile-projects.js')).__test__ as any;

describe('linkedin profile-projects adapter', () => {
  const command = findAdapter('linkedin', 'profile-projects');
  ...
  it('normalizes default and explicit profile URLs', () => {
    ...
    expect(normalizeProfileUrl('https://www.linkedin.com/in/gauravsaxena1997?x=1')).toBe(
      'https://www.linkedin.com/in/gauravsaxena1997?x=1',
    );
    expect(profileProjectsUrl('https://www.linkedin.com/in/gauravsaxena1997?x=1')).toBe(
      'https://www.linkedin.com/in/gauravsaxena1997/details/projects/',
    );
  });
  ...
  it('rejects non-profile URLs', () => {
    ...
    expect(() => profileProjectsUrl('https://www.linkedin.com/in/me/')).toThrow(
      CommandExecutionError,
    );
  });
...
Skills: JavaScript, Browser Automation`,
  ...
  it('parses a LinkedIn projects section rendered as one section', () => {
    const rows = parseProjectsSectionText(
      `Projects
...
Who your viewers also viewed`,
  ...
  it('normalizes rows and requires a title', () => {
    ...
    expect(normalizeProject({ rank: '2', title: ' Moniqo ', urls: ' https://example.com ' })).toMatchObject(
      { rank: 2, title: 'Moniqo', urls: 'https://example.com/' },
    );
    expect(
      normalizeProject({ title: ' Moniqo ', urls: 'javascript:alert(1) | https://example.com/demo' }),
  ...
  it('decodes LinkedIn safety redirect URLs', () => {
    expect(
      decodeLinkedInSafetyUrl(
        'https://www.linkedin.com/safety/go/?url=https%3A%2F%2Fgithub.com%2Fjackwener%2FOpenCLI&urlhash=x',
      ),
