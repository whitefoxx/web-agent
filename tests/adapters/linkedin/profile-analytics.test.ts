/**
 * Port of opencli's clis/linkedin/profile-analytics.test.js.
 */
...
import '../../../marketplace/linkedin/profile-analytics.js';

const { normalizeProfileAnalyticsUrl, parseMetric, parseDashboardMetrics, normalizeAnalytics } = (
  await import('../../../marketplace/linkedin/profile-analytics.js')
).__test__ as any;

describe('linkedin profile-analytics adapter', () => {
  const command = findAdapter('linkedin', 'profile-analytics');
  ...
  it('normalizes profile url default and explicit /in URL', () => {
    ...
    expect(normalizeProfileAnalyticsUrl('https://www.linkedin.com/in/gauravsaxena1997/')).toBe(
      'https://www.linkedin.com/in/gauravsaxena1997/',
    );
  });

  it('rejects non-profile URLs', () => {
    expect(() => normalizeProfileAnalyticsUrl('https://www.linkedin.com/jobs/')).toThrow(
  ...
  it('parses compact dashboard metrics', () => {
    ...
    expect(
      parseDashboardMetrics(
        '31 post impressions 23 search appearances 32 profile views 1,234 followers 500 connections',
      ),
    ).toEqual({
      profile_views: '32',
      post_impressions: '31',
  ...
  it('does not emit an all-empty analytics row', () => {
    expect(() =>
      normalizeAnalytics({ profile_url: 'https://www.linkedin.com/in/me/', raw_analytics: '' }),
