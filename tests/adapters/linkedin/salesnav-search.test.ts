/**
 * Port of opencli's clis/linkedin/salesnav-search.test.js.
 */
import { describe, expect, it } from 'vitest';
import { CommandExecutionError } from '@base/runtime/errors.js';
import '../../../marketplace/linkedin/salesnav-search.js';
...
} = (await import('../../../marketplace/linkedin/salesnav-search.js')).__test__ as any;
...
describe('linkedin salesnav-search command', () => {
  ...
  it('derives a profile URL from the sales-profile entityUrn token', () => {
    expect(profileUrlFromEntityUrn('urn:li:fs_salesProfile:(ACwAAAJS8TABxyz,NAME_SEARCH,Enlo)')).toBe(
      'https://www.linkedin.com/in/ACwAAAJS8TABxyz',
    );
  ...
  it('derives a Sales Navigator lead URL from the full sales-profile entityUrn', () => {
    expect(leadUrlFromEntityUrn('urn:li:fs_salesProfile:(ACwAAAJS8TABxyz,NAME_SEARCH,Enlo)')).toBe(
    ...
    const json = {
      elements: [
        ...
        },
        {
          ...
          currentPositions: [],
          pastPositions: [{ title: 'Past QA Lead', companyName: 'Old Co' }],
        },
      ],
    };
  ...
  it('fails typed on malformed lead payloads instead of silently dropping rows', () => {
    ...
    expect(() => parseLeads({ elements: [{ fullName: 'No Identity' }] })).toThrow(
