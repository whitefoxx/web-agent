/**
 * Port of opencli's clis/linkedin/connect.test.js.
 *
 * connect helpers are exported via the bundled adapter's `__test__`. The
 * command.func tests use self-contained fake pages (goto/wait/evaluate),
 * routing evaluate by script content just like opencli did.
 */
...
import '../../../marketplace/linkedin/connect.js';
...
const {
  ...
  clampNote,
  assessProfileSafety,
} = (await import('../../../marketplace/linkedin/connect.js')).__test__ as any;

function makeFakePage(
  probe: unknown,
  sendResult: unknown = { ok: true, status: 'sent', reason: 'connection_request_sent' },
) {
  return {
...
function makeSequentialFakePage(values: unknown[]) {
  let index = 0;
  return {
    goto: vi.fn(async () => undefined),
    wait: vi.fn(async () => undefined),
...
describe('linkedin connect helpers', () => {
  it('normalizes names and profile URLs', () => {
    ...
    expect(canonicalizeLinkedInProfileUrl('https://www.linkedin.com/in/jane/?mini=true#x')).toBe(
      'https://www.linkedin.com/in/jane/',
    );
    expect(canonicalizeLinkedInProfileUrl('https://ca.linkedin.com/in/jane/?mini=true#x')).toBe(
      'https://www.linkedin.com/in/jane/',
    );
  ...
  it('only accepts LinkedIn invitation route hrefs for sending', () => {
    expect(canonicalizeLinkedInInviteUrl('/preload/custom-invite/?vanityName=jane')).toBe(
      'https://www.linkedin.com/preload/custom-invite/?vanityName=jane',
    );
    expect(canonicalizeLinkedInInviteUrl('https://www.linkedin.com/feed/')).toBe('');
    expect(
      canonicalizeLinkedInInviteUrl(
        'https://evil-linkedin.com/preload/custom-invite/?vanityName=jane',
      ),
    ).toBe('');
  });

  it('unwraps browser bridge evaluate envelopes', () => {
    expect(unwrapEvaluateResult({ session: 'site:linkedin:1', data: { ok: true } })).toEqual({
      ok: true,
    });
    const raw = { ok: true };
  ...
  it('fails closed on wrong profile name, pending state, or missing connect button', () => {
    expect(
      assessProfileSafety(
        { name: 'Jane Doe', url: 'https://www.linkedin.com/in/jane/', connectAvailable: true },
        'Janet Doe',
        'https://www.linkedin.com/in/jane/',
      ).blockReason,
    ...
    expect(
      assessProfileSafety(
        { name: 'Jane Doe', url: 'https://www.linkedin.com/in/jane/' },
        'Jane Doe',
        'https://www.linkedin.com/in/jane/',
      ).blockReason,
  ...
  it('classifies routine non-connectable profiles separately from unsafe blocks', () => {
    expect(
      assessProfileSafety(
        { name: 'Jane Doe', url: 'https://www.linkedin.com/in/jane/', alreadyConnected: true },
        'Jane Doe',
        'https://www.linkedin.com/in/jane/',
      ),
    ).toMatchObject({
      ok: false,
      safety: 'routine_non_connectable',
      connectable: false,
      blockReason: 'already_connected',
    });
    expect(
      assessProfileSafety(
        { name: 'Jane Doe', url: 'https://www.linkedin.com/in/jane/', pending: true },
        'Jane Doe',
        'https://www.linkedin.com/in/jane/',
      ),
    ).toMatchObject({
      ...
      safety: 'routine_non_connectable',
      connectable: false,
      blockReason: 'connection_pending',
    });
    expect(
      assessProfileSafety(
        { name: 'Wrong Person', url: 'https://www.linkedin.com/in/wrong/', connectAvailable: true },
        'Jane Doe',
        'https://www.linkedin.com/in/jane/',
      ),
    ).toMatchObject({
      ...
      safety: 'unsafe_block',
      connectable: null,
      blockReason: 'profile_name_mismatch',
    });
  });

  it('passes only when profile url, name, and connect affordance all match', () => {
    const result = assessProfileSafety(
      {
        name: 'Jane Doe',
        url: 'https://www.linkedin.com/in/jane/?mini=true',
        connectAvailable: true,
      },
      'Jane Doe',
      'https://www.linkedin.com/in/jane/',
...
describe('linkedin connect command', () => {
  it('registers as a write command and dry-runs by default', async () => {
    const command = findAdapter('linkedin', 'connect');
    ...
    const page = makeFakePage({
      ...
      url: 'https://www.linkedin.com/in/jane/',
      connectAvailable: true,
      connectHref: '/preload/custom-invite/?vanityName=jane',
  ...
  it('blocks send when the connect link is not LinkedIn invitation route', async () => {
    ...
    const page = makeFakePage({
      ...
      url: 'https://www.linkedin.com/in/jane/',
      connectAvailable: true,
      connectHref: 'https://www.linkedin.com/feed/',
      buttonLabels: ['Connect'],
    });
  ...
  it('sends only when --send is true after verification and sent-invitations confirms delivery', async () => {
    ...
    expect(rows[0]).toMatchObject({
      ...
      recipient: 'Jane Doe',
      reason: 'sent_invitation_verified',
  ...
  it('retries sent-invitations verification before reporting unverified', async () => {
    ...
    expect(rows[0]).toMatchObject({
      ...
      reason: 'sent_invitation_verified',
      delivery_verified: true,
      matched_invitation_name: 'Jane Doe, P.Eng.',
    });
    expect(page.goto).toHaveBeenCalledWith(
      'https://www.linkedin.com/mynetwork/invitation-manager/sent/',
    );
  ...
  it('does not report sent when sent-invitations verification fails after retries', async () => {
    ...
    expect(rows[0]).toMatchObject({
      ...
      recipient: 'Jane Doe',
      reason: 'sent_invitation_not_found_after_retries',
