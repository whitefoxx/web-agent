/**
 * Port of opencli's clis/linkedin/jobs-preferences.test.js.
 * Pure-helper assertions via the bundled adapter's __test__.
 */
import { describe, expect, it } from 'vitest';
import { findAdapter } from '../../../src/runtime/registry.js';
import { CommandExecutionError } from '../../../src/runtime/errors.js';
import '../../../marketplace/linkedin/jobs-preferences.js';

const { inferOpenToWork, normalizePreferences } = (
  await import('../../../marketplace/linkedin/jobs-preferences.js')
).__test__ as any;

describe('linkedin jobs-preferences adapter', () => {
  const command = findAdapter('linkedin', 'jobs-preferences');
  ...
  it('normalizes preferences and alerts', () => {
    const out = normalizePreferences(
      {
        raw_preferences: 'Open to work turned off',
  ...
  it('rejects malformed payloads', () => {
    ...
    expect(() => normalizePreferences({}, null)).toThrow(CommandExecutionError);
    expect(() => normalizePreferences({ raw_preferences: '' }, { raw_preferences: '' })).toThrow(
