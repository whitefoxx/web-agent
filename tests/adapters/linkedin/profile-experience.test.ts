...
const {
  ...
  normalizeExperience,
} = (await import('../../../marketplace/linkedin/profile-experience.js')).__test__ as any;

describe('linkedin profile-experience adapter', () => {
  const command = findAdapter('linkedin', 'profile-experience');
  ...
  it('normalizes default and explicit profile URLs', () => {
    ...
    expect(profileExperienceUrl('https://www.linkedin.com/in/gauravsaxena1997?x=1')).toBe(
      'https://www.linkedin.com/in/gauravsaxena1997/details/experience/',
    );
  });
  ...
  it('rejects non-profile URLs', () => {
    ...
    expect(() => profileExperienceUrl('https://www.linkedin.com/in/me/')).toThrow(
      CommandExecutionError,
    );
  });
...
Skills: Generative AI, Next.js, TypeScript`,
  ...
  it('parses a LinkedIn experience section rendered as one section', () => {
    const rows = parseExperienceSectionText(
      `Experience
...
Who your viewers also viewed`,
  ...
  it('normalizes rows and requires a title', () => {
    ...
    expect(
      normalizeExperience({
        ...
        title: ' AI Engineer ',
        company: ' Self Employed ',
        skill_url: ' https://example.com/skills ',
      }),
    ).toMatchObject({
      rank: 2,
      total_count: 4,
  ...
  it('decodes LinkedIn safety redirect URLs', () => {
    expect(
      decodeLinkedInSafetyUrl(
        'https://www.linkedin.com/safety/go/?url=https%3A%2F%2Fexample.com%2Fdemo&urlhash=x',
      ),
