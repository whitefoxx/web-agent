/**
 * Port of opencli's clis/twitter/list-create.test.js.
 *
 * The bundled file re-exports `buildListCreateRow` + `parseListCreateArgs` by name.
 */
...
import { buildListCreateRow, parseListCreateArgs } from '../../../marketplace/twitter/list-create.js';

import '../../../marketplace/twitter/list-create.js';
...
function createPayload(overrides: Record<string, unknown> = {}) {
  return {
    ...
    bodyJson: {
      data: {
        list: { id_str: '123456789', name: 'My List', description: 'A list', mode: 'Private', ...overrides },
      },
    },
  };
}

describe('twitter list-create registration (marketplace)', () => {
  const cmd = () => findAdapter('twitter', 'list-create');
  ...
  it('registers the list-create command with the expected shape', () => {
    ...
    const nameArg = cmd()?.args?.find((a: { name: string }) => a.name === 'name');
    expect(nameArg).toBeTruthy();
    expect(nameArg?.required).toBe(true);
    expect(nameArg?.positional).toBe(true);
    const modeArg = cmd()?.args?.find((a: { name: string }) => a.name === 'mode');
    expect(modeArg?.default).toBe('public');
    const descArg = cmd()?.args?.find((a: { name: string }) => a.name === 'description');
  ...
  it('reads ct0 from cookies and unwraps Browser Bridge mutation envelopes', async () => {
    ...
    expect(rows).toEqual([
      { id: '123456789', name: 'My List', description: 'A list', mode: 'private', status: 'success' },
    ]);
  });
  ...
  it('keeps non-fatal GraphQL errors when a valid created list payload exists', () => {
    const row = buildListCreateRow({
      result: {
        ...createPayload(),
        bodyJson: { ...createPayload().bodyJson, errors: [{ message: 'DecodeException' }] },
      },
      name: 'My List',
      description: 'A list',
      mode: 'private',
  ...
  it('fails typed when the mutation response lacks post-condition evidence', () => {
    for (const result of [
      ...
      expect(() =>
        buildListCreateRow({ result, name: 'My List', description: '', mode: 'public' }),
      ).toThrow(CommandExecutionError);
    }
  });
