/**
 * Port of opencli's clis/twitter/bookmark-folder.test.js.
 */
import { describe, expect, it, vi } from 'vitest';
import { findAdapter } from '../../../src/runtime/registry.js';
import { __test__ } from '../../../marketplace/twitter/bookmark-folder.js';

import '../../../marketplace/twitter/bookmark-folder.js';

const { parseBookmarkFolderTimeline, extractFolderTweet, buildFolderTimelineUrl, FOLDER_ID_PATTERN } = __test__;

describe('twitter bookmark-folder URL builder (marketplace)', () => {
...
describe('twitter bookmark-folder timeline parser (marketplace)', () => {
            ...
            instructions: [
              {
                ...
                entries: [
                  ...
                  },
                  {
                    ...
                    content: { __typename: 'TimelineTimelineCursor', cursorType: 'Bottom', value: 'NEXT_CURSOR' },
                  },
                ],
              },
            ],
...
describe('twitter bookmark-folder id validation (marketplace)', () => {
...
describe('twitter bookmark-folder command (marketplace)', () => {
  const command = findAdapter('twitter', 'bookmark-folder');
  ...
  it('throws ArgumentError on unsafe folder-id before navigation', async () => {
    ...
    await expect(command!.func!(page, { 'folder-id': 'folder/123', limit: 5 })).rejects.toThrow(
      /Invalid folder-id/,
    );
  ...
  it('throws ArgumentError on empty folder-id', async () => {
    const page = { goto: vi.fn(), wait: vi.fn(), evaluate: vi.fn() };
    await expect(command!.func!(page, { 'folder-id': '   ', limit: 5 })).rejects.toThrow(
      /Invalid folder-id/,
    );
  });
  ...
  it('throws ArgumentError on invalid limit before navigation', async () => {
    for (const limit of [0, -1, 1.5, Number.NaN]) {
      const page = { goto: vi.fn(), wait: vi.fn(), evaluate: vi.fn() };
      await expect(command!.func!(page, { 'folder-id': '12345', limit })).rejects.toThrow(
        /Invalid --limit/,
      );
      expect(page.goto).not.toHaveBeenCalled();
    }
  ...
  it('throws AuthRequiredError when ct0 cookie is missing', async () => {
    ...
    await expect(command!.func!(page, { 'folder-id': '12345', limit: 5 })).rejects.toThrow(
  ...
  it('accepts an opaque safe folder-id and sends it in the GraphQL variables', async () => {
    ...
    expect(page.getCookies).toHaveBeenCalledWith({ url: 'https://x.com' });
    const fetchScript = page.evaluate.mock.calls[1][0] as string;
