/**
 * Port of opencli's clis/twitter/device-follow.test.js.
 */
...
import { __test__ } from '../../../marketplace/twitter/device-follow.js';

import '../../../marketplace/twitter/device-follow.js';

const { buildDeviceFollowUrl, extractEntries, joinEntryToTweet, shapeRow, parseDeviceFollow, parseLimit } =
  __test__;

function tweet(id: string, userId: string, overrides: Record<string, unknown> = {}) {
  return {
    id_str: id,
    user_id_str: userId,
    full_text: `text-${id}`,
...
function user(id: string, screenName: string) {
  return { id_str: id, screen_name: screenName };
}

function entry(tweetId: string) {
  return { entryId: `tweet-${tweetId}`, content: { item: { content: { tweet: { id: tweetId } } } } };
}

function nonTweetEntry(id = 'cursor-bottom') {
  return { entryId: id, content: { item: { content: {} } } };
}
...
describe('twitter device-follow (marketplace)', () => {
  const cmd = () => findAdapter('twitter', 'device-follow');
  ...
  it('parseDeviceFollow maps the legacy v1.1 payload to rows', () => {
    ...
    expect(rows?.rows.map((r: { author: string }) => r.author)).toEqual(['alice', 'bob']);
    expect(rows?.rows.every((r: { views: unknown }) => r.views === null)).toBe(true);
    expect(rows?.rows[0].url).toBe('https://x.com/alice/status/1');
  });

  it('parseDeviceFollow dedupes via the seen set', () => {
    const p = payload({ '1': tweet('1', 'u1') }, { u1: user('u1', 'alice') }, [entry('1'), entry('1')]);
  ...
  it('parseDeviceFollow tracks tweet entries that cannot join to required user identity', () => {
    ...
    expect(parsed).toMatchObject({ rows: [], entryCount: 1, unmatchedTweetEntries: 1, malformedEntries: 0 });
  });
  ...
  it('parseDeviceFollow tracks non-empty entries without tweet identity as parser drift', () => {
    ...
    expect(parsed).toMatchObject({ rows: [], entryCount: 1, unmatchedTweetEntries: 0, malformedEntries: 1 });
  });
  ...
  it('maps auth-coded non-2xx response to AuthRequiredError', async () => {
    const page = {
  ...
  it('throws CommandExecutionError for malformed or unjoinable device-follow payloads', async () => {
    ...
    await expect(
      cmd()!.func!(
        { getCookies: vi.fn().mockResolvedValue(cookies), evaluate: vi.fn().mockResolvedValue({}) },
        { limit: 5 },
      ),
    ).rejects.toBeInstanceOf(CommandExecutionError);
    await expect(
  ...
  it('returns parsed rows when the fetch succeeds', async () => {
    ...
    const rows = (await cmd()!.func!(page, { limit: 5 })) as Array<{ author: string }>;
  ...
  it('respects --limit when the upstream returns more than asked', async () => {
    ...
    const rows = (await cmd()!.func!(page, { limit: 3 })) as unknown[];
