/**
 * Port of opencli's clis/twitter/bookmark.test.js.
 */
...
import '../../../marketplace/twitter/bookmark.js';

describe('twitter bookmark command (marketplace)', () => {
  const cmd = findAdapter('twitter', 'bookmark');

  it('navigates to the tweet URL and reports success when the bookmark script confirms', async () => {
    expect(cmd?.func).toBeTypeOf('function');
    const page = createPageMock([{ ok: true, message: 'Tweet successfully bookmarked.' }]);
    ...
    expect(result).toEqual([{ status: 'success', message: 'Tweet successfully bookmarked.' }]);
  });
