/**
 * Port of opencli's clis/twitter/profile.test.js.
 */
...
import { __test__ } from '../../../marketplace/twitter/profile.js';

import '../../../marketplace/twitter/profile.js';

describe('twitter profile command (marketplace)', () => {
  const command = findAdapter('twitter', 'profile');
  ...
  it('throws typed when the profile result is structurally malformed', () => {
    ...
    expect(() => __test__.mapTwitterProfileResult({}, 'jack')).toThrow(CommandExecutionError);
    expect(() => __test__.mapTwitterProfileResult({ __typename: 'UserUnavailable' }, 'jack')).toThrow(
      CommandExecutionError,
    );
    expect(() => __test__.mapTwitterProfileResult({ legacy: {}, core: {} }, 'jack')).toThrow(
      CommandExecutionError,
    );
  });
  ...
  it('rejects invalid explicit usernames before navigation', async () => {
    ...
    await expect(command!.func!(page, { username: 'viewer/extra' })).rejects.toBeInstanceOf(
      ArgumentError,
    );
  ...
  it('maps GraphQL auth and not-found envelopes to typed failures', async () => {
    ...
    await expect(
      command!.func!(createPage({ ok: false, auth: true, error: 'HTTP 401' }), { username: 'jack' }),
    ).rejects.toBeInstanceOf(AuthRequiredError);
    await expect(
      command!.func!(createPage({ ok: false, notFound: true, error: 'User @missing not found' }), {
        username: 'missing',
      }),
    ).rejects.toBeInstanceOf(EmptyResultError);
    await expect(
      command!.func!(createPage({ session: 'site:twitter', data: [] }), { username: 'jack' }),
