/**
 * Port of opencli's clis/twitter/likes.test.js.
 *
 * Mechanism adaptation: opencli's EmptyResultError carried a `.hint` field;
 * ours folds the hint text into `.message`, so the privacy-message assertion
 * checks `.message`. The substantive coverage (privacy message + type) holds.
 */
...
import { __test__ } from '../../../marketplace/twitter/likes.js';

import '../../../marketplace/twitter/likes.js';
                        ...
                        itemContent: {
                          tweet_results: {
                            result: {
                              ...
                              core: {
                                user_results: { result: { legacy: { screen_name: 'alice', name: 'Alice' } } },
                              },
                            },
                          },
                        },
...
describe('twitter likes helpers (marketplace)', () => {
...
describe('twitter likes command — private timeline (marketplace)', () => {
  it('throws EmptyResultError with privacy message when API returns empty-timeline shape', async () => {
    const command = findAdapter('twitter', 'likes');
    const makePage = () => ({
      ...
      evaluate: vi.fn(async (script: unknown) => {
        ...
        if (text.includes('/Likes')) {
          return {
            ...
            data: { data: { user: { result: { __typename: 'User', timeline: {} } } } },
          };
        }
        throw new Error(`Unexpected evaluate: ${text.slice(0, 80)}`);
      }),
    });
    await expect(command!.func!(makePage(), { username: 'simonw', limit: 5 })).rejects.toMatchObject({
      message: expect.stringContaining('Likes are private by default on X'),
    });
    await expect(command!.func!(makePage(), { username: 'simonw', limit: 5 })).rejects.toBeInstanceOf(
      EmptyResultError,
    );
  });
});

describe('twitter likes command (marketplace)', () => {
  const command = () => findAdapter('twitter', 'likes');
  ...
  it('rejects invalid explicit username before cookies or navigation', async () => {
    ...
    await expect(command()!.func!(page, { username: 'viewer/extra', limit: 10 })).rejects.toBeInstanceOf(
      ArgumentError,
    );
  ...
  it('unwraps Browser Bridge envelopes for default-self user lookup and likes payload', async () => {
    const page = {
      ...
      evaluate: vi.fn(async (script: unknown) => {
        ...
        if (text.includes('operationName')) return null;
        if (text.includes('/UserByScreenName')) return { session: 'site:twitter', data: '42' };
        if (text.includes('/Likes')) return { session: 'site:twitter', data: likesPayload() };
        throw new Error(`Unexpected evaluate: ${text.slice(0, 80)}`);
      }),
    };

    const rows = (await command()!.func!(page, { limit: 1 })) as Array<Record<string, unknown>>;
    expect(rows).toHaveLength(1);
    expect(rows[0]).toMatchObject({ id: '1', author: 'alice', text: 'liked post' });
    const likesCall =
      page.evaluate.mock.calls.find(([script]) => String(script).includes('/Likes')) || [];
