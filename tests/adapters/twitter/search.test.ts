/**
 * Port of opencli's clis/twitter/search.test.js.
 */
import { describe, expect, it, vi } from 'vitest';
import { findAdapter } from '../../../src/runtime/registry.js';
import { __test__ } from '../../../marketplace/twitter/search.js';

import '../../../marketplace/twitter/search.js';
...
describe('twitter search command (marketplace)', () => {
  const command = () => findAdapter('twitter', 'search');

  function makeSearchPage(data: unknown) {
    return {
      getCookies: vi.fn().mockResolvedValue([{ name: 'ct0', value: 'csrf' }]),
      goto: vi.fn().mockResolvedValue(undefined),
      evaluate: vi.fn().mockResolvedValueOnce(null).mockResolvedValueOnce(data),
    };
  }
  ...
  it('fetches SearchTimeline directly instead of relying on SPA navigation', async () => {
                              ...
                              core: {
                                user_results: {
                                  result: {
                                    ...
                                    legacy: { description: 'Search author bio' },
                                  },
                                },
                              },
                              views: { count: '12' },
    ...
    const searchFetch = page.evaluate.mock.calls[1][0] as string;
  ...
  it('paginates past the old five-page cap until the requested limit is reached', async () => {
    ...
    const result = (await command()!.func!(page, { query: 'opencli', limit: 7 })) as Array<{ id: string }>;
...
describe('twitter search filter helpers (marketplace)', () => {
  describe('buildSearchQuery', () => {
    ...
    it('composes multiple filter clauses in stable order: query → from → has → exclude', () => {
      expect(
        buildSearchQuery('hot take', { from: '@alice', has: 'media', exclude: 'retweets' }),
...
describe('twitter search end-to-end with new filters (marketplace)', () => {
  const command = () => findAdapter('twitter', 'search');
  ...
  it('encodes the composed query and product=live into the GraphQL request', async () => {
    ...
    const searchFetch = evaluate.mock.calls[1][0] as string;
    expect(searchFetch).toContain('\\"product\\":\\"Latest\\"');
    expect(searchFetch).toContain(
      '\\"rawQuery\\":\\"breaking news from:alice filter:images -filter:nativeretweets\\"',
    );
  });

  it('throws ArgumentError when query and all filters are empty', async () => {
    const page = { goto: vi.fn().mockResolvedValue(undefined), evaluate: vi.fn() };
    await expect(command()!.func!(page, { query: '   ', limit: 5 })).rejects.toThrow(/empty/i);
  ...
  it('throws ArgumentError for invalid --from before navigation', async () => {
    ...
    await expect(
      command()!.func!(page, { query: 'hi', from: 'alice filter:links', limit: 5 }),
  ...
  it('throws ArgumentError for invalid --limit before navigation', async () => {
    const page = { goto: vi.fn(), evaluate: vi.fn() };
    await expect(command()!.func!(page, { query: 'hi', limit: 0 })).rejects.toThrow(/--limit/);
    expect(page.goto).not.toHaveBeenCalled();
  });
