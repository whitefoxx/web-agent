/**
 * Port of opencli's clis/twitter/tweets.test.js.
 */
import { describe, expect, it, vi } from 'vitest';
import { findAdapter } from '@base/runtime/registry.js';
import { ArgumentError, AuthRequiredError } from '@base/runtime/errors.js';
import { __test__ } from '../../../marketplace/twitter/tweets.js';

import '../../../marketplace/twitter/tweets.js';

describe('twitter tweets helpers (marketplace)', () => {
  const cmd = () => findAdapter('twitter', 'tweets');
  ...
  it('makes the username argument optional so it can default to the logged-in user', () => {
    const usernameArg = cmd()?.args?.find((arg: { name: string }) => arg.name === 'username');
  ...
  it('detects the logged-in user via AppTabBar_Profile_Link when no username is given', async () => {
    ...
    const page = {
      ...
      evaluate: vi.fn(async (script: unknown) => {
        const text = typeof script === 'function' ? (script as () => void).toString() : String(script);
                                ...
                                itemContent: {
                                  tweet_results: {
                                    result: {
                                      ...
                                      core: {
                                        user_results: {
                                          result: { legacy: { screen_name: 'viewer', name: 'Viewer' } },
                                        },
                                      },
                                    },
                                  },
    ...
    const rows = (await cmd()!.func!(page, { limit: 1 })) as Array<Record<string, unknown>>;
