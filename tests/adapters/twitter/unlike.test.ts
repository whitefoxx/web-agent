/**
 * Port of opencli's clis/twitter/unlike.test.js.
 */
...
import '../../../marketplace/twitter/unlike.js';

describe('twitter unlike command (marketplace)', () => {
  const cmd = findAdapter('twitter', 'unlike');

  it('navigates to the tweet URL and reports success when the unlike script confirms', async () => {
    expect(cmd?.func).toBeTypeOf('function');
    const page = createPageMock([{ ok: true, message: 'Tweet successfully unliked.' }]);
    ...
    expect(result).toEqual([{ status: 'success', message: 'Tweet successfully unliked.' }]);
  });
    ...
    const page = createPageMock([
      {
        ok: false,
        message:
          'Could not find the Unlike button on this tweet after waiting 10 seconds. Are you logged in?',
      },
    ]);
    const result = await cmd!.func!(page, {
      url: 'https://x.com/alice/status/2040254679301718161',
  ...
  it('rejects invalid tweet URLs before navigation', async () => {
    const page = createPageMock([]);
    await expect(
      cmd!.func!(page, { url: 'https://x.com/alice/status/2040254679301718161/photo/1' }),
