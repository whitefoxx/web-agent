/**
 * Port of opencli's clis/notebooklm/generate-audio.test.js.
 *
 * generate-audio.js re-exports its pure helpers via `__test__`
 * (AUDIO_OVERVIEW_CONFIG_BLOCK / buildCreateAudioArgs / parseAudioIdFromResult),
 * so the wire-format and tree-walk assertions port verbatim. The `--execute`
 * refusal test runs against the live `command.func`.
 */
import { describe, expect, it, vi } from 'vitest';
import { ArgumentError } from '@base/runtime/errors.js';
import { findAdapter } from '@base/runtime/registry.js';

import { __test__ } from '../../../marketplace/notebooklm/generate-audio.js';

const { AUDIO_OVERVIEW_CONFIG_BLOCK, buildCreateAudioArgs, parseAudioIdFromResult } = __test__;

describe('notebooklm generate-audio (marketplace)', () => {
  ...
  it('parseAudioIdFromResult skips notebook/source ids before selecting the generated audio id', () => {
    ...
    expect(parseAudioIdFromResult([notebookId, [[sourceId]], [audioId]], [notebookId, sourceId])).toBe(
      audioId,
    );
  });

  it('refuses to trigger remote audio generation without --execute', async () => {
    const command = findAdapter('notebooklm', 'generate-audio');
