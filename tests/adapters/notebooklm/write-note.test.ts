/**
 * Port of opencli's clis/notebooklm/write-note.test.js.
 *
 * write-note.js re-exports its pure helpers via `__test__`
 * (parseNoteTitle / parseNoteContent / buildCreateNoteShellArgs /
 * buildMutateNoteArgs / parseNoteIdFromResult), so all helper assertions port
 * verbatim. The `--execute` refusal test runs against the live `command.func`.
 */
import { describe, expect, it, vi } from 'vitest';
import { ArgumentError } from '@base/runtime/errors.js';
import { findAdapter } from '@base/runtime/registry.js';

import { __test__ } from '../../../marketplace/notebooklm/write-note.js';

const { parseNoteTitle, parseNoteContent, buildCreateNoteShellArgs, buildMutateNoteArgs, parseNoteIdFromResult } =
  __test__;

describe('notebooklm write-note (marketplace)', () => {
  ...
  it('buildCreateNoteShellArgs matches the HAR-verified wire format', () => {
    expect(buildCreateNoteShellArgs('nb-123')).toEqual(['nb-123', '', [1], null, 'New Note', null, [2]]);
  });
  ...
  it('refuses to create a remote note without --execute', async () => {
    const command = findAdapter('notebooklm', 'write-note');
