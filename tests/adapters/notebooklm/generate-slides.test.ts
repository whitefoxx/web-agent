/**
 * Port of opencli's clis/notebooklm/generate-slides.test.js.
 *
 * generate-slides.js re-exports its pure helpers via `__test__`
 * (SLIDE_DECK_CONFIG_BLOCK / buildCreateSlidesArgs / parseSlideDeckLength /
 * parseSlidesIdFromResult), so all helper assertions port verbatim. The
 * `--execute` refusal test runs against the live `command.func`.
 */
import { describe, expect, it, vi } from 'vitest';
import { ArgumentError } from '@base/runtime/errors.js';
import { findAdapter } from '@base/runtime/registry.js';

import { __test__ } from '../../../marketplace/notebooklm/generate-slides.js';

const { SLIDE_DECK_CONFIG_BLOCK, buildCreateSlidesArgs, parseSlideDeckLength, parseSlidesIdFromResult } =
  __test__;

describe('notebooklm generate-slides (marketplace)', () => {
  ...
  it('parseSlidesIdFromResult skips notebook/source ids before selecting the generated deck id', () => {
    ...
    expect(parseSlidesIdFromResult([notebookId, [[sourceId]], [slidesId]], [notebookId, sourceId])).toBe(
      slidesId,
    );
  });

  it('refuses to trigger remote slide generation without --execute', async () => {
    const command = findAdapter('notebooklm', 'generate-slides');
    const page = { goto: vi.fn() };
    await expect(
      command!.func!(page, { notebook: '17e2b882-6a01-4c6c-9262-0738dfa2abee' }),
