/**
 * Port of opencli's clis/notebooklm/create.test.js.
 *
 * The bundled create.js re-exports its pure helpers via `__test__`
 * (parseCreateTitle / parseCreateEmoji / parseCreateProjectResult), so the
 * helper-level assertions port verbatim. The `--execute` refusal test runs
 * against the live `command.func` via findAdapter.
 */
import { describe, expect, it, vi } from 'vitest';
import { ArgumentError } from '@base/runtime/errors.js';
import { findAdapter } from '@base/runtime/registry.js';

import { __test__ } from '../../../marketplace/notebooklm/create.js';

const { parseCreateTitle, parseCreateEmoji, parseCreateProjectResult } = __test__;

describe('notebooklm create (marketplace)', () => {
  ...
  it('parseCreateProjectResult extracts the notebook id from the singleton-wrapped RPC result', () => {
    const result = [
      [['notebook-payload-prefix', null, 'ec806f5b-fe74-4588-8f77-f073b91e9b1e', 'opencli-test', '🧪']],
    ];
  ...
  it('refuses to create a remote notebook without --execute', async () => {
    const command = findAdapter('notebooklm', 'create');
