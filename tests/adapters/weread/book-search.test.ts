/**
 * Port of opencli's clis/weread/book-search.test.js.
 *
 * The bundled marketplace adapter is a faithful inline of the opencli source.
 * Its `func` takes ONLY `args` (no page) and does ALL networking through the
 * global `fetch`, exactly the seam opencli stubs. So porting is pure mechanism
 * swap:
 *   - getRegistry().get('weread/book-search') → findAdapter('weread', 'book-search')
 *   - register-by-importing the bundled file under marketplace/
 *   - error codes asserted via toMatchObject map 1:1 to our error classes'
 *     `.code` (ARGUMENT / EMPTY_RESULT / COMMAND_EXEC)
 * Every substantive assertion (fetch URL ordering/contents, the rendered
 * markdown, book-rank selection, searchIdx pagination, fail-closed branches)
 * is preserved verbatim from opencli.
 */
import { beforeEach, describe, expect, it, vi } from 'vitest';
import { findAdapter } from '../../../src/runtime/registry.js';

import '../../../marketplace/weread/book-search.js';
...
describe('weread/book-search (marketplace)', () => {
  const command = findAdapter('weread', 'book-search');
      ...
      .mockResolvedValueOnce(
        textResponse(
          readerHtml({
            ...
            title: '史记',
            author: '司马迁',
            chapters: [{ chapterUid: 171, chapterIdx: 9, title: '卷一 五帝本纪第一' }],
          }),
        ),
      )
      .mockResolvedValueOnce(
  ...
  it('uses book-rank to choose among search results and paginates by searchIdx', async () => {
    ...
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        jsonResponse({
          result: [{ chapterUid: 2, abstract: 'third', searchIdx: 3 }],
          hasMore: 0,
        }),
      );
    vi.stubGlobal('fetch', fetchMock);
    ...
    const result = (await command!.func!({
      ...
      limit: 3,
      raw: true,
    })) as Array<{ snippet: string; book_id: string }>;
  ...
  it('rejects off-domain reader URLs instead of fetching arbitrary pages', async () => {
    ...
    await expect(
      command!.func!({ book: 'https://example.com/web/reader/reader-1', query: '舜' }),
  ...
  it('fails closed when in-book search results are malformed', async () => {
    ...
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(jsonResponse({ result: [{ searchIdx: 1 }], hasMore: 0 })),
    );
  ...
  it('fails closed when in-book pagination does not advance', async () => {
    ...
    await expect(
      command!.func!({ book: '123', query: '舜', limit: 2, raw: true }),
    ).rejects.toMatchObject({
      code: 'COMMAND_EXEC',
  ...
  it('fails closed when a full in-book search page has no pagination state', async () => {
    ...
    await expect(
      command!.func!({ book: '123', query: '舜', limit: 51, raw: true }),
    ).rejects.toMatchObject({
      code: 'COMMAND_EXEC',
  ...
  it('validates numeric arguments instead of silently clamping', async () => {
    expect(command?.func).toBeTypeOf('function');
    await expect(
      command!.func!({ book: '史记', query: '舜', limit: 101 }),
    ).rejects.toMatchObject({
      code: 'ARGUMENT',
