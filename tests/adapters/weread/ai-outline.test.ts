/**
 * Port of opencli's clis/weread/ai-outline.test.js.
 *
 * The bundled marketplace adapter is a faithful inline of the opencli source:
 * it networks via the global `fetch` and reads cookies via `page.getCookies`.
 * So the only porting changes vs. opencli are MECHANISM swaps:
 *   - getRegistry().get('weread/ai-outline')  → findAdapter('weread', 'ai-outline')
 *   - register-by-importing the bundled file under marketplace/
 * `vi.stubGlobal('fetch', ...)` and the `page.getCookies` mocking are kept
 * verbatim from opencli (the seams are identical), and the substantive
 * assertions (AUTH_REQUIRED mapping, raw rows + depth filtering, Cookie header,
 * which endpoints are POSTed) are preserved unchanged.
 */
import { beforeEach, describe, expect, it, vi } from 'vitest';
import { findAdapter } from '@base/runtime/registry.js';

import '../../../marketplace/weread/ai-outline.js';

describe('weread ai-outline (marketplace)', () => {
  const command = findAdapter('weread', 'ai-outline');
  ...
  it('returns structured rows for --raw and respects depth filtering', async () => {
      ...
      .mockResolvedValueOnce({
        ...
        json: () =>
          Promise.resolve({
            data: [
              {
                updated: [{ chapterUid: 'c1', title: '第一章' }],
              },
            ],
          }),
      })
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      'https://weread.qq.com/web/book/outline',
      expect.objectContaining({
        method: 'POST',
      }),
    );
