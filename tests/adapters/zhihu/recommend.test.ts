/**
 * Port of opencli's clis/zhihu/recommend.test.js.
 *
 * Read adapter: page.goto then page.evaluate(<fetch IIFE>) returning the parsed
 * feed JSON (or a { __httpError } sentinel). opencli mocks page.evaluate
 * directly and asserts the in-page script contains the feed endpoint; we keep
 * that exact seam.
 */
import { describe, expect, it, vi } from 'vitest';
import { findAdapter } from '@base/runtime/registry.js';
import { AuthRequiredError, CliError } from '@base/runtime/errors.js';
import '../../../marketplace/zhihu/recommend.js';

describe('zhihu recommend (marketplace)', () => {
  it('returns recommendations from the Zhihu feed API', async () => {
    const cmd = findAdapter('zhihu', 'recommend');
      ...
      evaluate: vi
        ...
        .mockResolvedValueOnce({
          data: [
            {
              id: '0_1',
              target: { id: 'a1', type: 'answer', author: { name: 'alice' }, question: { id: 'q1', title: 'first' } },
            },
            {
              id: '1_1',
              target: { id: 'a2', type: 'answer', author: { name: 'bob' }, question: { id: 'q2', title: 'second' } },
            },
          ],
          paging: {
            is_end: false,
        ...
        .mockResolvedValueOnce({
          data: [
            {
              id: '1_1',
              target: { id: 'a2', type: 'answer', author: { name: 'bob duplicate' }, question: { id: 'q2', title: 'duplicate' } },
            },
