/**
 * Port of opencli's clis/zhihu/comment.test.js.
 *
 * Write adapter, same seam as answer: identity evaluate then API-POST evaluate.
 */
import { describe, expect, it, vi } from 'vitest';
import { findAdapter } from '@base/runtime/registry.js';
import '../../../marketplace/zhihu/comment.js';

describe('zhihu comment (marketplace)', () => {
  it('registers as a cookie browser command', () => {
    const cmd = findAdapter('zhihu', 'comment');
  ...
  it('creates a comment via API and returns result', async () => {
    ...
    expect(rows).toEqual([expect.objectContaining({ outcome: 'created', author_identity: 'alice' })]);
  });
  ...
  it('requires the comment API response to include the created id', async () => {
    ...
    const page = {
      ...
      evaluate: vi
        ...
        .mockResolvedValueOnce({
          ok: false,
          status: 200,
          message: 'Comment API response did not include a created comment id',
        }),
    };
    await expect(
      cmd!.func!(page, { target: 'answer:1:2', text: 'hello', execute: true }),
    ...
    expect(page.evaluate.mock.calls[1][0]).toContain(
      'Comment API response did not include a created comment id',
    );
  });
});
