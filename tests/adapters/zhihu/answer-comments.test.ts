/**
 * Port of opencli's clis/zhihu/answer-comments.test.js.
 *
 * Read adapter: page.goto → optional page.getCurrentUrl → page.evaluate(<comments
 * fetch IIFE>). opencli mocks page.evaluate directly and asserts the in-page
 * script contains the comments endpoint; same seam here. The bundled file
 * re-exports __test__ (parseAnswerTarget, normalizeCommentsApiUrl, buildRows).
 */
...
import { __test__ as helpers } from '../../../marketplace/zhihu/answer-comments.js';

import '../../../marketplace/zhihu/answer-comments.js';

describe('zhihu answer-comments (marketplace)', () => {
  it('registers as a cookie read command', () => {
    const cmd = findAdapter('zhihu', 'answer-comments');
  ...
  it('returns flattened comments while limiting only top-level comments', async () => {
    ...
    const evaluate = vi.fn().mockImplementation(async (js) => {
      ...
      return {
        ...
        paging: {
          is_end: false,
          next: 'https://www.zhihu.com/api/v4/answers/2036567240334653053/comments?offset=20',
        },
      };
    });
    const page = {
      ...
      getCurrentUrl: vi
        .fn()
        .mockResolvedValue(
          'https://www.zhihu.com/question/2022852734622114542/answer/2036567240334653053',
        ),
      evaluate,
    };
    await expect(
  ...
  it('follows paging.next until enough top-level comments are collected', async () => {
    ...
    const evaluate = vi
      ...
      .mockResolvedValueOnce({
        data: [{ id: 'c2', author: { member: { id: 'u3', name: 'carol' } }, content: 'second' }],
        paging: { is_end: true },
      });
    const page = { goto: vi.fn().mockResolvedValue(undefined), evaluate };
    const rows = (await cmd!.func!(page, { id: '1', limit: 2, 'replies-limit': 0 })) as Array<{
      id: string;
    }>;
  ...
  it('supports typed answer targets', async () => {
    ...
    const page = {
      ...
      evaluate: vi.fn().mockResolvedValue({
        data: [{ id: 'c1', author: { member: { id: 'u1', name: 'alice' } }, content: 'typed target comment' }],
        paging: { is_end: true },
      }),
    };
    await expect(
      cmd!.func!(page, { id: 'answer:2022852734622114542:2036567240334653053', limit: 1, 'replies-limit': 0 }),
  ...
  it('maps auth failures to AuthRequiredError', async () => {
    ...
    await expect(cmd!.func!(page, { id: '1', limit: 1, 'replies-limit': 0 })).rejects.toBeInstanceOf(
      AuthRequiredError,
    );
  });
  ...
  it('rejects malformed pagination next URLs and repeated next URLs', async () => {
    ...
    await expect(
      cmd!.func!(malformedNextPage, { id: '1', limit: 2, 'replies-limit': 0 }),
    ...
    const repeatedNextPage = {
      ...
      evaluate: vi.fn().mockResolvedValue({
        ...
        paging: {
          is_end: false,
          next: 'https://www.zhihu.com/api/v4/answers/1/comments?order=normal&limit=20&offset=0&status=open',
        },
      }),
    };
    await expect(
      cmd!.func!(repeatedNextPage, { id: '1', limit: 2, 'replies-limit': 0 }),
    ).rejects.toBeInstanceOf(CommandExecutionError);
  });
  ...
  it('rejects null comment items and non-primitive comment ids', async () => {
    const cmd = findAdapter('zhihu', 'answer-comments');
    const basePage = (data: unknown) => ({
      goto: vi.fn().mockResolvedValue(undefined),
      evaluate: vi.fn().mockResolvedValue({ data, paging: { is_end: true } }),
    });
    await expect(
      cmd!.func!(basePage([null]), { id: '1', limit: 1, 'replies-limit': 0 }),
    ).rejects.toBeInstanceOf(CommandExecutionError);
    await expect(
      cmd!.func!(basePage([{ id: { value: 'c1' }, content: 'object id' }]), {
        id: '1',
        limit: 1,
        'replies-limit': 0,
      }),
    ...
    await expect(
      cmd!.func!(basePage([{ id: true, content: 'boolean id' }]), {
        id: '1',
        limit: 1,
        'replies-limit': 0,
      }),
  ...
  it('rejects invalid inputs before navigation', async () => {
    ...
    await expect(
      cmd!.func!(page, { id: 'not-an-answer', limit: 1, 'replies-limit': 0 }),
    ).rejects.toBeInstanceOf(ArgumentError);
    await expect(
      cmd!.func!(page, { id: '1', limit: 0, 'replies-limit': 0 }),
    ).rejects.toBeInstanceOf(ArgumentError);
    await expect(
      cmd!.func!(page, { id: '1', limit: 1001, 'replies-limit': 0 }),
    ).rejects.toBeInstanceOf(ArgumentError);
    await expect(
      cmd!.func!(page, { id: '1', limit: 1, 'replies-limit': -1 }),
    ).rejects.toBeInstanceOf(ArgumentError);
    await expect(
      cmd!.func!(page, { id: '1', limit: 1, 'replies-limit': 101 }),
    ).rejects.toBeInstanceOf(ArgumentError);
...
describe('zhihu answer-comments helpers (marketplace)', () => {
  it('parseAnswerTarget handles exact input shapes', () => {
    ...
    expect(helpers.parseAnswerTarget('https://www.zhihu.com/question/10/answer/123')).toEqual({
      answerId: '123',
      questionId: '10',
    });
    expect(helpers.parseAnswerTarget('https://zhihu.com/answer/123?utm=1')).toEqual({
      answerId: '123',
      questionId: '',
    });
  ...
  it('normalizeCommentsApiUrl only accepts same-answer Zhihu comments API URLs', () => {
    expect(
      helpers.normalizeCommentsApiUrl('https://www.zhihu.com/api/v4/answers/123/comments?offset=20', '123'),
    ).toBe('https://www.zhihu.com/api/v4/answers/123/comments?offset=20');
    expect(
      helpers.normalizeCommentsApiUrl('https://api.zhihu.com/answers/123/comments?offset=20', '123'),
    ).toBe('https://www.zhihu.com/api/v4/answers/123/comments?offset=20');
    expect(
      helpers.normalizeCommentsApiUrl('https://www.zhihu.com/api/v4/answers/999/comments?offset=20', '123'),
    ).toBe('');
    expect(
      helpers.normalizeCommentsApiUrl('https://evil.example/api/v4/answers/123/comments?offset=20', '123'),
    ).toBe('');
  });
    ...
    const rows = helpers.buildRows(
      [
        ...
        },
        {
          id: 'r2',
          author: { member: { id: 'u3', name: 'carol' } },
          reply_to_author: { member: { id: 'u2', name: 'bob' } },
          content: 'nested',
        },
      ],
      { answerId: 'a1', questionId: 'q1', topLevelLimit: 1, repliesLimit: 5 },
    ).rows;
    expect(
      rows.map((row: { id: string; parent_id: string; depth: number; comment_rank: number; reply_rank: number }) => [
        row.id,
        row.parent_id,
        row.depth,
        row.comment_rank,
