/**
 * Port of opencli's clis/zhihu/question.test.js.
 *
 * Read adapter: page.goto then page.evaluate(<answers fetch IIFE>). opencli
 * mocks page.evaluate directly; same seam here.
 */
import { describe, expect, it, vi } from 'vitest';
import { findAdapter } from '@base/runtime/registry.js';
import { AuthRequiredError, CliError } from '@base/runtime/errors.js';
import '../../../marketplace/zhihu/question.js';

describe('zhihu question (marketplace)', () => {
  it('returns answers from the Zhihu API', async () => {
    const cmd = findAdapter('zhihu', 'question');
  ...
  it('supports created-time sorting', async () => {
    ...
    const page = { goto, evaluate };
    await expect(
      cmd!.func!(page, { id: '2021881398772981878', limit: 1, sort: 'created' }),
    ).resolves.toEqual([
  ...
  it('maps auth-like answer failures to AuthRequiredError', async () => {
    ...
    await expect(cmd!.func!(page, { id: '2021881398772981878', limit: 3 })).rejects.toBeInstanceOf(
      AuthRequiredError,
    );
  });
  ...
  it('rejects non-numeric question IDs', async () => {
    ...
    await expect(cmd!.func!(page, { id: "abc'; alert(1); //", limit: 1 })).rejects.toBeInstanceOf(
      CliError,
    );
  ...
  it('rejects invalid limits before navigation', async () => {
    const cmd = findAdapter('zhihu', 'question');
    const page = { goto: vi.fn(), evaluate: vi.fn() };
    await expect(cmd!.func!(page, { id: '2021881398772981878', limit: 0 })).rejects.toBeInstanceOf(
      CliError,
    );
    expect(page.goto).not.toHaveBeenCalled();
    expect(page.evaluate).not.toHaveBeenCalled();
  ...
  it('rejects excessive limits before navigation', async () => {
    const cmd = findAdapter('zhihu', 'question');
    const page = { goto: vi.fn(), evaluate: vi.fn() };
    await expect(cmd!.func!(page, { id: '2021881398772981878', limit: 1001 })).rejects.toBeInstanceOf(
      CliError,
    );
    expect(page.goto).not.toHaveBeenCalled();
    expect(page.evaluate).not.toHaveBeenCalled();
  ...
  it('rejects invalid sort before navigation', async () => {
    ...
    const page = { goto: vi.fn(), evaluate: vi.fn() };
    await expect(
      cmd!.func!(page, { id: '2021881398772981878', limit: 1, sort: 'unknown' }),
