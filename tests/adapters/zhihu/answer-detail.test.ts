/**
 * Port of opencli's clis/zhihu/answer-detail.test.js.
 *
 * Read adapter: page.goto → optional page.getCurrentUrl → page.evaluate(<answer
 * fetch IIFE>). opencli mocks page.evaluate directly; same seam here. The
 * bundled file re-exports __test__ (stripHtml, extractAnswerId,
 * parseAnswerTarget, extractQuestionIdFromAnswerUrl).
 */
...
import { __test__ as helpers } from '../../../marketplace/zhihu/answer-detail.js';

import '../../../marketplace/zhihu/answer-detail.js';

describe('zhihu answer-detail (marketplace)', () => {
  it('registers as a cookie read command', () => {
    const cmd = findAdapter('zhihu', 'answer-detail');
  ...
  it('fetches a single answer by numeric id and returns one row', async () => {
    ...
    const evaluate = vi.fn().mockImplementation(async (js) => {
      ...
      return {
        ...
        question: {
          id: 630517537,
          title: '回想自己的人生阅历，你最想教给孩子们的一个道理是什么？',
        },
      };
    });
    const page = { goto, evaluate };
    const rows = (await cmd!.func!(page, { id: '1937205528846655537', 'max-content': 0 })) as Array<
  ...
  it('accepts a full Zhihu answer URL as id, preserving full id precision', async () => {
    ...
    expect(rows[0].url).toBe(
      'https://www.zhihu.com/question/2021881398772981878/answer/1937205528846655537',
    );
  ...
  it('accepts the typed-target form answer:<qid>:<aid>', async () => {
    ...
    const page = { goto: vi.fn().mockResolvedValue(undefined), evaluate };
    const rows = (await cmd!.func!(page, {
      id: 'answer:2021881398772981878:999',
      'max-content': 0,
    })) as Array<Record<string, unknown>>;
  ...
  it('uses the redirected canonical URL as question id source for bare answer ids', async () => {
    ...
    const page = {
      ...
      getCurrentUrl: vi
        .fn()
        .mockResolvedValue('https://www.zhihu.com/question/2021881398772981878/answer/999'),
    ...
    const rows = (await cmd!.func!(page, { id: '999', 'max-content': 0 })) as Array<
      Record<string, unknown>
    >;
    expect(rows[0].question_id).toBe('2021881398772981878');
  ...
  it('returns the full stripped body when --max-content is 0 (default)', async () => {
    ...
    const page = { goto: vi.fn().mockResolvedValue(undefined), evaluate };
    const rows = (await cmd!.func!(page, { id: '1', 'max-content': 0 })) as Array<{
      content: string;
    }>;
  ...
  it('respects --max-content as an opt-in cap', async () => {
    ...
    const page = { goto: vi.fn().mockResolvedValue(undefined), evaluate };
    const rows = (await cmd!.func!(page, { id: '1', 'max-content': 100 })) as Array<{
      content: string;
    }>;
  ...
  it('falls back to bare /answer/<id> URL when the response is missing question metadata', async () => {
    ...
    const page = { goto: vi.fn().mockResolvedValue(undefined), evaluate };
    const rows = (await cmd!.func!(page, { id: '42', 'max-content': 0 })) as Array<
      Record<string, unknown>
    >;
  ...
  it('maps 401/403 to AuthRequiredError', async () => {
    ...
    await expect(cmd!.func!(page, { id: '1', 'max-content': 0 })).rejects.toBeInstanceOf(
      AuthRequiredError,
    );
  });
  ...
  it('rejects non-numeric answer ids before navigation', async () => {
    ...
    await expect(cmd!.func!(page, { id: "abc'; alert(1); //", 'max-content': 0 })).rejects.toBeInstanceOf(
      ArgumentError,
    );
  ...
  it('rejects negative --max-content before navigation', async () => {
    const cmd = findAdapter('zhihu', 'answer-detail');
    const page = { goto: vi.fn(), evaluate: vi.fn() };
    await expect(cmd!.func!(page, { id: '1', 'max-content': -5 })).rejects.toBeInstanceOf(
      ArgumentError,
    );
    expect(page.goto).not.toHaveBeenCalled();
  });
...
describe('zhihu answer-detail helpers (marketplace)', () => {
  ...
  it('parseAnswerTarget handles exact input shapes', () => {
    ...
    expect(helpers.parseAnswerTarget('https://zhihu.com/answer/123?utm=1#x')).toEqual({
      answerId: '123',
      questionId: '',
    });
