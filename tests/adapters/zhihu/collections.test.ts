/**
 * Port of opencli's clis/zhihu/collections.test.js.
 *
 * Read adapter: page.goto → page.evaluate(me) → page.evaluate(collections list).
 * opencli sequences page.evaluate by call count; same seam here. The bundled
 * adapter imports @jackwener/opencli/logger (aliased to our shim) — mocked.
 */
...
import '../../../marketplace/zhihu/collections.js';

describe('zhihu collections (marketplace)', () => {
  it('returns list of collections', async () => {
    const cmd = findAdapter('zhihu', 'collections');
    ...
    const evaluate = vi
      ...
      .mockResolvedValueOnce({
        ...
        paging: {
          totals: 3,
          is_end: false,
          next: 'https://www.zhihu.com/api/v4/people/testuser/collections?offset=1&limit=1',
        },
      })
      .mockResolvedValueOnce({
        data: [
