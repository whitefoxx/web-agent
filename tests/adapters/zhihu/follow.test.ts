/**
 * Port of opencli's clis/zhihu/follow.test.js.
 *
 * Write adapter with no identity resolution — single page.evaluate POST.
 */
import { describe, expect, it, vi } from 'vitest';
import { findAdapter } from '@base/runtime/registry.js';
import '../../../marketplace/zhihu/follow.js';

describe('zhihu follow (marketplace)', () => {
  it('registers as a cookie browser command', () => {
    const cmd = findAdapter('zhihu', 'follow');
  ...
  it('uses the parsed user slug for user follow API calls', async () => {
    ...
    expect(page.evaluate.mock.calls[0][0]).toContain(
      "'https://www.zhihu.com/api/v4/members/' + targetId + '/followers'",
    );
