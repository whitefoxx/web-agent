/**
 * Port of opencli's clis/zhihu/collection.test.js.
 *
 * Read adapter: page.goto then page.evaluate(<collections/items fetch IIFE>).
 * The bundled adapter imports @jackwener/opencli/logger (aliased to our shim);
 * we mock it the same way opencli did so log.info() output stays quiet.
 */
...
import '../../../marketplace/zhihu/collection.js';

describe('zhihu collection (marketplace)', () => {
  it('returns collection items from the Zhihu API', async () => {
    const cmd = findAdapter('zhihu', 'collection');
  ...
  it('maps auth failures to AuthRequiredError', async () => {
    ...
    await expect(cmd!.func!(page, { id: '83283292', offset: 0, limit: 20 })).rejects.toBeInstanceOf(
      AuthRequiredError,
    );
  });
  ...
  it('respects pagination offset', async () => {
    ...
    expect(evaluate).toHaveBeenCalledWith(expect.stringContaining('offset=40'));
  });
  ...
  it('rejects invalid offset and limit before navigation', async () => {
    ...
    await expect(cmd!.func!(page, { id: '83283292', offset: -1, limit: 20 })).rejects.toBeInstanceOf(
      ArgumentError,
    );
    await expect(cmd!.func!(page, { id: '83283292', offset: 0, limit: 0 })).rejects.toBeInstanceOf(
      ArgumentError,
    );
    expect(page.goto).not.toHaveBeenCalled();
  });
  ...
  it('paginates until requested limit and deduplicates items', async () => {
    ...
    const evaluate = vi
      ...
      .mockResolvedValueOnce({
        ...
        paging: {
          totals: 3,
          is_end: false,
          next: 'https://www.zhihu.com/api/v4/collections/83283292/items?offset=1&limit=1',
        },
      })
      .mockResolvedValueOnce({
        data: [
    ...
    expect(result.map((row: { title: string }) => row.title)).toEqual(['A', 'B']);
  ...
  it('skips items with missing content.type instead of failing the page (F-23)', async () => {
    const cmd = findAdapter('zhihu', 'collection');
    const evaluate = vi.fn().mockResolvedValue({
      data: [
        {
    ...
    const page = { goto: vi.fn().mockResolvedValue(undefined), evaluate };
    // F-23: an unknown/missing content.type (e.g. a zvideo) is dropped (return null),
    // not thrown — so one odd item can't blank the whole heterogeneous page.
    await expect(cmd!.func!(page, { id: '83283292', offset: 0, limit: 20 })).resolves.toEqual([]);
  });

  it('skips supported items missing title/url identity instead of failing the page (F-23)', async () => {
    const cmd = findAdapter('zhihu', 'collection');
    const page = {
      goto: vi.fn().mockResolvedValue(undefined),
      evaluate: vi.fn().mockResolvedValue({
    ...
    // F-23: a malformed item (no title/url) is dropped rather than throwing.
    await expect(cmd!.func!(page, { id: '83283292', offset: 0, limit: 20 })).resolves.toEqual([]);
  });
});
