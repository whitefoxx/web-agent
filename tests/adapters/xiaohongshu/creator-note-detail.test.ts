/**
 * Port of opencli's clis/xiaohongshu/creator-note-detail.test.js.
 *
 * The bundled marketplace/xiaohongshu/creator-note-detail.js re-exports the
 * pure helpers `parseCreatorNoteDetailText`, `parseCreatorNoteDetailDomData`,
 * `appendAudienceRows`, `appendTrendRows`, so those tests are ported directly.
 * The func tests install their own script-content `page.evaluate` routers
 * (exactly as opencli did), so the port only swaps the registry lookup and the
 * error imports; the page mock provides the rest of the surface (goto/wait/…).
 */
...
import '../../../marketplace/xiaohongshu/creator-note-detail.js';
import {
  appendAudienceRows,
  appendTrendRows,
  parseCreatorNoteDetailDomData,
  parseCreatorNoteDetailText,
} from '../../../marketplace/xiaohongshu/creator-note-detail.js';

describe('xiaohongshu/creator-note-detail (marketplace)', () => {
  const getCommand = () => findAdapter('xiaohongshu', 'creator-note-detail');
...
粉丝占比 0%`;
  ...
  it('appends audience source and portrait rows from API payloads', () => {
    const rows = appendAudienceRows([], {
        ...
        source: [
          {
            ...
            info: { imp_count: 1469, view_count: 276, interaction_count: 15 },
          },
        ],
      },
      audienceSourceDetail: {
        ...
        age: [{ title: '25-34', value: 55 }],
        city: [{ title: '上海', value: 8 }],
        interest: [{ title: '二次元', value: 13 }],
      },
    });
    expect(rows).toEqual([
  ...
  it('appends trend summary rows from hour/day series payloads', () => {
    const rows = appendTrendRows([], {
      audienceTrend: { no_data: true, no_data_tip_msg: '数据统计中，请稍后查看' },
      noteBase: {
        hour: {
          view_list: [
          ...
          like_list: [{ date: new Date('2026-03-18T21:00:00+08:00').getTime(), count: 2 }],
        },
        day: {
          view_list: [{ date: new Date('2026-03-18T00:00:00+08:00').getTime(), count: 307 }],
        },
      },
    });
    expect(rows).toEqual([
  ...
  it('navigates to the note detail page and returns parsed rows', async () => {
    ...
    const result = await getCommand()!.func!(page, { 'note-id': 'demo-note-id' });
  ...
  it('polls the capture buffer while the dashboard fires its signed datacenter/note/* requests', async () => {
    ...
    await getCommand()!.func!(page, { 'note-id': 'demo-note-id' });
    expect(page.wait.mock.calls.length).toBeGreaterThanOrEqual(4);
    const captureProbeCalls = page.evaluate.mock.calls.filter(([script]: [unknown]) =>
      String(script).includes('JSON.stringify(window.__xhsCapture'),
    );
  ...
  it('matches signed API captures by exact pathname so source/detail cannot shadow source', async () => {
    const domData = {
      ...
      infoText: '示例笔记\n2026-03-19 12:00\n切换笔记',
      sections: [
        { title: '基础数据', metrics: [{ label: '曝光数', value: '100', extra: '粉丝占比 10%' }] },
      ],
    };
    const detailCapture = [
    ...
    const baseCapture = [
      'https://creator.xiaohongshu.com/api/galaxy/creator/datacenter/note/base?note_id=demo-note-id',
      {
        ...
        body: JSON.stringify({
          data: {
            hour: { view_list: [{ date: new Date('2026-03-19T12:00:00+08:00').getTime(), count: 7 }] },
          },
        }),
      },
    ];
    ...
    for (const orderedCaptures of [
      ...
      const captureMap = Object.fromEntries(orderedCaptures as Array<[string, unknown]>);
      const page = makeFakeXiaohongshuPage();
      page.evaluate = vi.fn(async (script: unknown) => {
        const s = String(script);
        if (s.includes('window.__xhsCapture =')) return undefined;
  ...
  it('throws a typed error when a captured signed API returns non-2xx', async () => {
    ...
    await expect(getCommand()!.func!(page, { 'note-id': 'demo-note-id' })).rejects.toBeInstanceOf(
