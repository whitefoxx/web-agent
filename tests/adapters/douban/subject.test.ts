...
import { ArgumentError, CliError } from '@base/runtime/errors.js';
...
describe('douban subject (marketplace)', () => {
  ...
  it('navigates to the movie subject page and returns the scraped detail in an array', async () => {
    // F-9: the in-page evaluate now returns the raw `fullTitle`; the func splits
    // it into title/originalTitle in Node scope (where normalizeText exists).
    const scraped = {
      id: '1234567',
      type: 'movie',
      fullTitle: '让子弹飞',
    ...
    page.extract.mockResolvedValueOnce(scraped);
