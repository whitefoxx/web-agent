...
/** One assistant turn that emits SEVERAL tool_calls at once (the model batching
 * independent steps). Each entry is [toolName, args]. */
function multiToolMsg(calls: [string, Record<string, unknown>?][]): ChatCompletionResponse {
  return {
    choices: [
      {
        message: {
...
describe('runApiSession — engine integration scenarios', () => {
  ...
  it('breaks out of a repeated failing tool (thrash) → checkpoint', async () => {
    ...
    expect(r.notices.some((t) => /tripped the breaker/.test(t))).toBe(true);
  });
  ...
  it('checkpoints when the step budget is exhausted', async () => {
    ...
    expect(r.completeCalls).toHaveLength(2);
    expect(r.doneReason).toBe('checkpoint');
    expect(r.notices.some((t) => /step limit/.test(t))).toBe(true);
  });
  ...
  it('plan mode: a rejected plan cancels without executing', async () => {
    ...
    expect(r.doneReason).toBe('no_more_commands');
    expect(r.notices.some((t) => /Canceled/.test(t))).toBe(true);
  });
  ...
  it('plan mode uses the read-only planning prompt + submit_plan tool', async () => {
    ...
    expect(String(sys.content)).toContain('Planning mode');
  ...
  it('spawn_subagent: runs isolated and returns only a digest', async () => {
    ...
    // v2: subagent progress is a structured `subagent` event, not a notice.
    expect(r.events.some((e) => e.type === 'subagent' && e.phase === 'done' && e.ok === true)).toBe(
      true,
    );
    expect(r.doneReason).toBe('no_more_commands');
  });
  ...
  it('plan mode: gives up nudging and lets the answer through instead of erroring (§10.17)', async () => {
    ...
    expect(r.notices.some((t) => /did not submit a confirmable plan/.test(t))).toBe(true);
  });
  ...
  it('mid-run re-plan: an interjection asking for a plan re-enters the approval gate (§10.16)', async () => {
    ...
    expect(r.notices.some((t) => /re-planning/.test(t))).toBe(true);
    expect(r.doneReason).toBe('no_more_commands');
  });
  ...
  it('plan mode: reflects once before finishing (reflect/re-plan)', async () => {
    ...
    expect(r.notices.some((t) => /reconcile/.test(t))).toBe(true);
    expect(r.doneReason).toBe('no_more_commands');
  });
  ...
  it('parallel reads: independent read tool_calls in one turn run concurrently, fold in order (v3 step 2)', async () => {
    // Register two READ adapters so lookupAdapter classifies them as parallelizable.
    cli({ site: 'par', name: 'a', description: 'read a', access: 'read', func: async () => 'A' });
    cli({ site: 'par', name: 'b', description: 'read b', access: 'read', func: async () => 'B' });

    let active = 0;
    let maxActive = 0;
    const exec = vi.fn(async (opts: { tool: string }) => {
      active++;
      maxActive = Math.max(maxActive, active);
      await new Promise((r) => setTimeout(r, 20));
      active--;
      return { ok: true, result: `${opts.tool}-result`, durationMs: 20 };
    });

    const r = await runScenario({
      responses: [multiToolMsg([['par__a'], ['par__b']]), textMsg('done')],
      executeTool: exec as unknown as EngineContext['executeTool'],
    });

    // Both ran, and OVERLAPPED (serial would cap concurrency at 1).
    expect(exec).toHaveBeenCalledTimes(2);
    expect(maxActive).toBe(2);

    // Trace stream proves overlap: both 'started' precede the first 'completed'.
    const phases = r.events
      .filter((e) => e.type === 'tool_trace')
      .map((e) => (e as { trace: { status: string } }).trace.status);
    const lastStarted = phases.lastIndexOf('started');
    const firstDone = phases.findIndex((s) => s === 'completed');
    expect(lastStarted).toBeLessThan(firstDone);

    // Both results folded into the next request, in original call order.
    const toolMsgs = r.completeCalls[1]!.messages.filter((m) => m.role === 'tool');
    expect(toolMsgs.map((m) => m.content)).toEqual(['par__a-result', 'par__b-result']);
