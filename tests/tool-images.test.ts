...
describe('stripDataUrls', () => {
  it('replaces base64 image blobs with a placeholder, leaving other text intact', () => {
    ...
    expect(stripDataUrls(text)).toBe('{"dataUrl":"[image omitted]","bytes":12}');
  });
  it('strips every data URL in the text', () => {
    const text = 'a data:image/png;base64,AAAA b data:image/jpeg;base64,BBBB c';
    expect(stripDataUrls(text)).toBe('a [image omitted] b [image omitted] c');
  });
  it('a labeler can emit RESOLVABLE tokens per blob (api-engine passes the image registry — §10.25)', () => {
    const text = 'a data:image/png;base64,AAAA b data:image/jpeg;base64,BBBB c';
    let n = 0;
    expect(stripDataUrls(text, () => `[img_${++n}]`)).toBe('a [img_1] b [img_2] c');
  });
});
