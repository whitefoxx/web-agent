...
import { findAdapter, unregister } from '@base/runtime/registry.js';
...
describe('installFromCaptured → registry wiring', () => {
  ...
  it('explore-origin installs into a separate namespace (prefixed site)', async () => {
    unregister('nsx', 'one');
    unregister('my-nsx', 'one');
    const r = await installFromCaptured(
      { source: 'src', defs: [pipelineDef('nsx', 'one')], origin: { type: 'explore' } },
      5_000,
    );
    expect(r.ok).toBe(true);
    // id + registry entry use the namespaced site, not the authored one.
    expect(r.id).toBe('my-nsx/one');
    expect(findAdapter('my-nsx', 'one')).toBeTruthy();
    expect(findAdapter('nsx', 'one')).toBeUndefined();
  });

  it('marketplace + explored adapters for the same site coexist (no shadowing)', async () => {
    unregister('coex', 'one');
    unregister('my-coex', 'one');
    await installFromCaptured(
      { source: 'm', defs: [pipelineDef('coex', 'one')], origin: { type: 'marketplace' } },
      6_000,
    );
    await installFromCaptured(
      { source: 'e', defs: [pipelineDef('coex', 'one')], origin: { type: 'explore' } },
      6_100,
    );
    // Both survive — the explored one didn't overwrite the marketplace one.
    expect(findAdapter('coex', 'one')).toBeTruthy();
    expect(findAdapter('my-coex', 'one')).toBeTruthy();
  });
});
