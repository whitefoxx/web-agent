...
describe('pipeline executor: single-object fetch → map (profile/user shape) — F-11', () => {
  const pipeline: Pipeline = [
    { fetch: { url: 'https://public.api.bsky.app/xrpc/app.bsky.actor.getProfile?actor=bsky.app' } },
    { map: { handle: '${{ item.handle }}', followers: '${{ item.followersCount }}' } },
  ];
  it('wraps a single-object fetch as ONE row so the following map transforms it', async () => {
    const impl: FetchImpl = async () => ({
      ok: true,
      status: 200,
      text: async () => JSON.stringify({ handle: 'bsky.app', displayName: 'Bluesky', followersCount: 42 }),
    });
    const { rows } = await runPipeline(pipeline, { args: {} }, { fetchImpl: impl });
    expect(rows).toEqual([{ handle: 'bsky.app', followers: 42 }]);
  });
});

describe('pipeline executor: fetch.params query string — F-6', () => {
  it('appends evaluated fetch.params as a query string (v2ex topic show.json)', async () => {
    const calls: string[] = [];
    const impl: FetchImpl = async (url) => {
      calls.push(url);
      return { ok: true, status: 200, text: async () => JSON.stringify([{ id: 1, title: 't' }]) };
    };
    const pipeline: Pipeline = [
      {
        fetch: {
    ...
    await runPipeline(pipeline, { args: { id: '1219312' } }, { fetchImpl: impl });
    expect(calls[0]).toBe('https://www.v2ex.com/api/topics/show.json?id=1219312');
  });

  it('skips undefined/empty params and merges with an existing query string', async () => {
    const calls: string[] = [];
    const impl: FetchImpl = async (url) => {
      calls.push(url);
    ...
    const pipeline: Pipeline = [
      {
        fetch: {
          ...
          params: { b: '${{ args.b }}', c: '${{ args.missing }}' },
        },
      },
    ];
    await runPipeline(pipeline, { args: { b: 'yes' } }, { fetchImpl: impl });
    expect(calls[0]).toBe('https://x.test/api?a=1&b=yes');
  });
});
...
describe('pipeline expr: sandbox hardening (Tier3-#15)', () => {
  it('legit Object.keys/values/entries still work', () => {
    expect(evaluateExpr('${{ Object.keys(item) }}', ctx({ row: { a: 1, b: 2 } }))).toEqual(['a', 'b']);
    expect(evaluateExpr('${{ Object.values(item) }}', ctx({ row: { a: 1, b: 2 } }))).toEqual([1, 2]);
  });

  it('Object reflection methods are NOT exposed (no route to Function ctor / prototype pollution)', () => {
    // The escape was: Object.getOwnPropertyDescriptor(
    //   Object.getPrototypeOf(Object.keys), 'constructor').value === Function.
    expect(() =>
      evaluateExpr('${{ Object.getOwnPropertyDescriptor(item, "x") }}', ctx({ row: {} })),
    ).toThrow();
    expect(() => evaluateExpr('${{ Object.getPrototypeOf(item) }}', ctx({ row: {} }))).toThrow();
    expect(() =>
      evaluateExpr('${{ Object.defineProperty(item, "x", item) }}', ctx({ row: {} })),
    ).toThrow();
    expect(() => evaluateExpr('${{ Object.create(item) }}', ctx({ row: {} }))).toThrow();
  });

  it('member access to constructor / __proto__ / prototype stays blocked', () => {
    expect(() => evaluateExpr('${{ item.constructor }}', ctx({ row: {} }))).toThrow();
    expect(() => evaluateExpr('${{ item.__proto__ }}', ctx({ row: {} }))).toThrow();
  });
});
