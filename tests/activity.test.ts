...
describe('toolActivity', () => {
  it('labels generic web primitives in Chinese with the right icon', () => {
    expect(
      toolActivity(T('generic__open_url', { url: 'https://youtube.com/results?q=x' })),
    ).toEqual({
      icon: 'navigate',
      label: '打开 youtube.com/results?q=x',
    });
    expect(toolActivity(T('generic__get_page_text')).icon).toBe('read');
    expect(toolActivity(T('generic__screenshot'))).toEqual({ icon: 'camera', label: '截图' });
    expect(toolActivity(T('generic__click_by_text', { text: '登录按钮' }))).toEqual({
      icon: 'click',
      label: '点击 "登录按钮"',
    });
  });
  ...
  it('labels engine pseudo-tools', () => {
    ...
    expect(toolActivity(T('spawn_subagent', { task: '抓取并对比' })).label).toContain('子任务');
    expect(toolActivity(T('remember', { fact: '常用账号 X' })).label).toContain('记住');
  });
