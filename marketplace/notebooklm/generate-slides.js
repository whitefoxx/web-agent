...
function parseSlideDeckLength(value) {
  ...
  if (!Number.isInteger(length) || length <= 0) {
    throw new ArgumentError2("--length must be a positive integer");
  }
  return length;
}
cli({
  ...
  func: async (page, kwargs) => {
    ...
    if (!slidesId) {
      throw new CommandExecutionError2("NotebookLM CreateArtifact (slides) RPC returned no slide-deck id; the server may have rejected the request.");
    }
    return [{
